...
lxc-info -n opencart || lxc-clone -s -B aufs trusty-base opencart

for d in juju mysql opencart; do
  lxc-start -d -n $d;
done
...
WP_IP=$(lxc-info -n opencart -i | awk '{print $2}')
...
test -d charms/trusty/opencart || git clone -b trusty https://github.com/ClodoCorp/charm-opencart.git charms/trusty/opencart
...
juju deploy --repository=charms/ local:trusty/opencart --to 2 || juju deploy --repository=charms/ local:trusty/opencart --to 2 || exit 1;
juju add-relation mysql opencart

juju expose opencart

juju deploy --repository=charms/ local:trusty/haproxy --to 1
juju add-relation haproxy opencart

for s in mysql opencart haproxy; do
    while true; do
